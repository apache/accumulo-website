...
that ships with the Accumulo tarball. The new API has the following benefits over the old API:
  ...
  * The new API does not required `ZooKeeperInstance` to be created first before creating a client.
  ...
  * Clients can now be created with default setting for `BatchWriter`, `Scanner`, etc.
