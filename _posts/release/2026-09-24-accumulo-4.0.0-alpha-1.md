...
Do not use this version in production. We advise that users start with a new instance to
understand new concepts, try new features and test their applications for the purpose of
finding and reporting issues. Upgrading an existing instance is not advised as the
upgrade code has not been thoroughly tested.
...
The 4.x user guide, located [here][4docs], is being updated. It will be in draft state until the final 4.0.0 release.
...
* {% ghi 6539 %} Using an underscore in a resource group name may cause the Monitor to fail.
